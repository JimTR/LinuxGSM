...
fn_stop_graceful_ctrlc(){
	...
	# Sends quit.
	tmux -L linuxgsm send-keys -t "${sessionname}" C-c  > /dev/null 2>&1
...
fn_stop_graceful_avorion(){
	...
	# Sends /save.
	tmux -L linuxgsm send-keys -t "${sessionname}" /save ENTER > /dev/null 2>&1
	sleep 5
	# Sends /quit.
	tmux -L linuxgsm send-keys -t "${sessionname}" /stop ENTER > /dev/null 2>&1
	# Waits up to 30 seconds giving the server time to shutdown gracefuly.
	for seconds in {1..30}; do
		check_status.sh
		if [ "${status}" == "0" ]; then
